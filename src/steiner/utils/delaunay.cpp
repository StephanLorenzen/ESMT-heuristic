...
Utils::Delaunay::Delaunay(std::vector<Utils::Point> &points) : Graph(points) {
...
void Utils::Delaunay::doDelaunayQHull() {
  doPrepInputQHull();
...
void Utils::Delaunay::doParseOutputQHull() {
  int d = this->dimension();
  this->simplices.clear();
  this->point_handles = std::vector<PointHandle>(this->n());
  
  std::ifstream outfile(this->qdelaunay_output);
  std::string buffer;
  ...
  if(outfile.is_open()) {
	...
	for(int k = j+1; k < d+1; k++) {
	  ...
	  if(flag.find(key) == flag.end()) {
	    flag[key] = true;
	    this->edges.push_back(Edge(i0,i1, Utils::length(this->getPoint(i0), this->getPoint((i1)))));
	  }
	}
      }
